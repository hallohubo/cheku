...
#define UMSocialInstance [UMSocialManager defaultManager]

@implementation HDUMHelper
...
+ (void)setupSocial{
    /* 打开调试日志 */
    [UMSocialInstance openLog:YES];
    /* 设置友盟appkey */
    [UMSocialInstance setUmSocialAppkey:@UmengAppKey];
...
+ (void)configUSharePlatforms
{
    ...
    [[UMSocialManager defaultManager] setPlaform:UMSocialPlatformType_WechatSession appKey:@WechatAppId appSecret:@WechatAppSecret redirectURL:@"http://www.liudu.com"];
    /*
     * 移除相应平台的分享，如微信收藏
     */
...
{
    [[UMSocialManager defaultManager] getUserInfoWithPlatform:platformType currentViewController:target completion:^(id result, NSError *error) {
        if (error) {
            block(nil, error);
            return ;
        }
...
+ (void)setupbeginLog:(NSString *)pageTitle {
    [MobClick beginLogPageView:pageTitle];
}

+ (void)setupEndLog:(NSString *)pageTitle {
