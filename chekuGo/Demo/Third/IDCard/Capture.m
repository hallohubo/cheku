...
- (void)captureStillImage:(void(^)(BOOL isSuc))block{
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection
{
        ...
        [outPutSetting isEqualToNumber:[NSNumber numberWithInt:kCVPixelFormatType_420YpCbCr8BiPlanarFullRange]]  )
    {
        if([delegate respondsToSelector:@selector(idCardRecognited:)])
        {
            ...
            {
                [self captureStillImage:^(BOOL isSuc) {
                    if (!isSuc) {
                        return ;
                    }
        ...
        else
        {
            ...
            while(i < ret){
                ...
                if(xlen){
                    ...
                    Dlog(@"ctype = %d, value = %@", ctype, [NSString stringWithCString:(char *)content encoding:gbkEncoding]);
                    if(ctype == 0x21)
