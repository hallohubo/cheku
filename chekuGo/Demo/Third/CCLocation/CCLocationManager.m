...
- (void)locationManager:(CLLocationManager *)manager didUpdateToLocation:(CLLocation *)newLocation fromLocation:(CLLocation *)oldLocation
{
    ...
    [geocoder reverseGeocodeLocation:newLocation completionHandler:^(NSArray *placemarks, NSError *error){
        ...
        if (placemarks.count > 0) {
            ...
            _lastCity = [NSString stringWithFormat:@"%@", placemark.locality];//,placemark.administrativeArea
            [standard setObject:_lastCity forKey:CCLastCity];//省市地址
            NSLog(@"______%@",_lastCity);
#define s(p, m) (p? p: m)
#define str(p) (p? p: @"")
            _lastAddress = [NSString stringWithFormat:@"%@%@%@%@%@%@%@", str(placemark.country), str(placemark.administrativeArea), str(placemark.locality), str(placemark.subLocality), str(placemark.thoroughfare), str(placemark.subThoroughfare), str(placemark.name)];//详细地址
            NSLog(@"______%@",_lastAddress);
            NSLog(@"%@,%@,%@,%@", placemark.name, placemark.administrativeArea, placemark.subAdministrativeArea, placemark.inlandWater);
            NSDictionary *dic = placemark.addressDictionary;
            NSLog(@"--%@", dic);
            NSArray *ar = dic[@"FormattedAddressLines"];
            if (ar.count > 0) {
                addressDesc = ar[0];
                NSLog(@"addressDesc = %@", addressDesc);
            }
        ...
        if (_addressBlock) {
            _addressBlock(s(addressDesc, _lastAddress));
            _addressBlock = nil;
        }
    }];
...
-(void)startLocation{
    if([CLLocationManager locationServicesEnabled] && [CLLocationManager authorizationStatus] != kCLAuthorizationStatusDenied){
        ...
        UIAlertView *alvertView = [[UIAlertView alloc]initWithTitle:@"提示" message:@"定位服务没开启, 请到设置->隐私, 打开软件的定位服务" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles: nil];
        [alvertView show];
    }
}
